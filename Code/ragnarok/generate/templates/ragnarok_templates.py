...
class RagnarokTemplates:
    def __init__(self, prompt_mode: PromptMode):
        ...
        self.system_message_claude = "This is a chat between a user and an artificial intelligence assistant. The assistant gives helpful and detailed answers to the user's question based on the context references. The assistant should also indicate when the answer cannot be found in the context references."
        self.system_message_claude_no_cite = "This is a chat between a user and an artificial intelligence assistant. The assistant gives helpful and detailed answers to the user's question."
    ...
    def __call__(self, query: str, context: List[str], model: str) -> List[str]:
        if not ("gpt" in model or "chatqa" in model.lower() or "claude" in model.lower()):
            self.sep = "\n"
        ...
        elif "gpt" in model or "claude" in model.lower():
            user_input_context = (
                f"Instruction: {self.get_instruction()}"
                + self.sep
        ...
        if "gpt" in model:
            ...
            system_message = (
                self.system_message_claude_no_cite
                if self.prompt_mode
                in [
                    PromptMode.RAGNAROK_V4_NO_CITE,
                    PromptMode.RAGNAROK_V5_BIOGEN_NO_CITE,
                ]
                else self.system_message_claude
            )
            return {
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": fix_text(user_input_context),
                            }
                        ],
                    }
                ],
