...
from generate.post_processor import GPTPostProcessor
    ...
    def run_llm_batched(
        ...
        try:
            if vllm:
                ...
                print("Running with transformers...")
                inputs = self._tokenizer(prompts, return_tensors="pt", padding=True).to(self._device)
                outputs = self._llm.generate(
                    **inputs,
                    max_new_tokens=self._output_token_estimate,
                    temperature=0.0,
                    do_sample=False,
                )
                responses = self._tokenizer.batch_decode(outputs, skip_special_tokens=True)
